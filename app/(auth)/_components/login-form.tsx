...
function LoginFormContent({ onSwitchToSignup, compact }: LoginFormProps) {
  ...
  if (mfaToken) {
    return (
      <div className="w-full">
        ...
        <Typography as="h1" variant={compact ? "h3" : "h2"}>
          Authenticator code
        </Typography>
        <Typography variant="muted" className="mt-2 text-sm sm:text-base">
              ...
              <InputOTPGroup className="gap-2">
                {OTP_SLOTS.map((index) => (
                  <InputOTPSlot
                    key={index}
                    index={index}
                    className="size-10 rounded-full border text-base first:rounded-full last:rounded-full sm:size-11"
                  />
                ))}
              </InputOTPGroup>
            </InputOTP>
    ...
    <div className="w-full">
      ...
      <div className="mt-6 sm:mt-8">
        <FormModified
          ...
          formKey={formKey}
          fieldsetProps={{ className: "space-y-4 sm:space-y-5" }}
          onSubmit={(values) => {
            login({
              path: AUTH_API.login,
              data: {
...
export default function LoginForm(props: LoginFormProps) {
  return (
    <Suspense
      fallback={<Loader variant="fetch" label="Loading..." className="py-16" />}
    >
